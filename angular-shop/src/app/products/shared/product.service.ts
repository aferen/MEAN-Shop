...
export class ProductService {
  ...
  public updateProduct(data: { product: Product; files: FileList }) {
    const url = `${this.productsUrl}/${data.product['_id']}`;
    ...
    const dbOperation = this.uploadService
      .startUpload(data).toPromise()
      .then((task) => {
        data.product.imageNames.push(task["data"].name);
        return data;
      })
      .then((dataWithImagePath) => {
        return this.http.put<Product>(this.helperService.getUrl(url), data.product)
        .toPromise()
        .then(
          response => { // Success
  ...
  public deleteProduct(product: Product) {
    ...
    //this.uploadService.deleteFile(product.imageNames);

    return this.http.delete<Product>(this.helperService.getUrl(url))
      .toPromise()
      .then(() => this.logSuccess('success deleting ' + product.name))
      .catch((error) => {
