...
const generateToken = (user) => {
  ...
  const expiration = Math.floor(Date.now() / 1000) + 60 * process.env.ACCESS_TOKEN_EXPIRATION_IN_MINUTES

  // returns signed and encrypted token
  return auth.access_token_encrypt(
    jwt.sign(
      {
        data: {
          _id: user
...
const generateRefreshToken = (user) => {
  // Gets expiration time
  const expiration = Math.floor(Date.now() / 1000) + 60 * process.env.REFRESH_TOKEN_EXPIRATION_IN_MINUTES

  // returns signed and encrypted token
  return auth.refresh_token_encrypt(
    jwt.sign(
      {
        data: {
          _id: user
...
const saveUserAccessAndReturnToken = async (req, user) => {
  return new Promise((resolve, reject) => {
    ...
    userAccess.save((err) => {
      ...
      const tokenInfo = setTokens(generateToken(user._id),generateRefreshToken(user._id))
...
const checkPermissions = async (data, next) => {
  return new Promise((resolve, reject) => {
    User.findById(data.id, (err, result) => {
      ...
      for (const element of result.role) {
        if (data.roles.indexOf(element) > -1) {
          return resolve(next())
        }
      }
...
const getUserIdFromToken = async (token) => {
  return new Promise((resolve, reject) => {
    ...
    jwt.verify(auth.access_token_decrypt(token), process.env.ACCESS_TOKEN_SECRET, (err, decoded) => {
        if (err) {
          if (err.expiredAt) {
              reject(utils.buildErrObject(401, err.name))  
          }       
...
const getUserIdFromRefreshToken = async (token) => {
  return new Promise((resolve, reject) => {
    // Decrypts, verifies and decode token
    jwt.verify(auth.refresh_token_decrypt(token), process.env.REFRESH_TOKEN_SECRET, (err, decoded) => {
        if (err) {
          if (err.expiredAt) {
              reject(utils.buildErrObject(401, err.name))  
          }       
...
exports.getRefreshToken = async (req, res) => {
  try {
    ...
    //   .replace('Bearer ', '')
    //   .trim()
    //let userId = await getUserIdFromRefreshToken(tokenEncrypted)
    let userId = await utils.isIDGood(req.user._id)
